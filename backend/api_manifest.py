...
def get_api_list(content_script_query: str = "") -> str:
    """Return the full list of all source names for the frontend.

    Shows ALL priority levels so the user can see the complete chain.
    """
    try:
        from settings.manager import get_ryuu_session, get_depotbox_sid, get_local_manifest_path, get_manifesthub_api_key
        ...
        # Priority 0: Local Manifest Folder
        local_path = get_local_manifest_path()
        if local_path:
            api_names.append({"name": "Local Folder", "index": -5})

        # Priority 1: TwentyTwo Cloud (free, always available)
        api_names.append({"name": "TwentyTwo Cloud", "index": -4})

        # Priority 2-3: Premium sources
        if get_ryuu_session():
            api_names.append({"name": "Ryuu Premium", "index": -3})
        if get_depotbox_sid():
            api_names.append({"name": "DepotBox Premium", "index": -2})

        # Priority 4: ManifestHub API (needs key)
        if get_manifesthub_api_key():
            api_names.append({"name": "ManifestHub API", "index": -1})

        # Priority 5: Free APIs from config
        ...
        # Priority 6: SLStools fallback sources
        api_names.append({"name": "Spinoza", "index": 900})
        api_names.append({"name": "Sadie", "index": 901})
        api_names.append({"name": "Sushi", "index": 902})

        # Priority 7: GitHub Repos (SDO — 19 repos)
        api_names.append({"name": "GitHub Repos (SDO)", "index": 950})
