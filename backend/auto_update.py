...
try:
    from security import safe_extract_archive, validate_zip_archive
except Exception:  # pragma: no cover - security module should be bundled
...
def _extract_update_zip(zip_path: str) -> None:
    """Validate and safely extract an update ZIP into the plugin directory."""
    if validate_zip_archive is not None:
        with open(zip_path, "rb") as fh:
            archive_bytes = fh.read()
        is_valid, error = validate_zip_archive(archive_bytes, "auto-update")
        if not is_valid:
            raise RuntimeError(f"Unsafe update archive: {error}")

    with zipfile.ZipFile(zip_path, "r") as archive:
        if safe_extract_archive is not None:
            safe_extract_archive(archive, get_plugin_dir())
        else:
            # Fallback for unexpected partial installs. The bundled security.py
            # path should normally be used.
...
def apply_pending_update_if_any() -> str:
    ...
    try:
        ...
        _extract_update_zip(pending_zip)
        try:
