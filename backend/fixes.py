...
except Exception:  # pragma: no cover - bundled in normal plugin installs
...
# ── Fixes index cache (avoids HEAD requests to R2, TTL: 5 min) ─────────
...
_FIXES_INDEX_TTL = 300          # seconds (5 min)  --  base TTL
_FIXES_INDEX_BACKOFF_UNTIL: float = 0  # epoch: don't retry before this time (429 backoff)
_FIXES_INDEX_BACKOFF_TTL = 600  # seconds (10 min)  --  backoff after 429
_HEAD_FALLBACK_WARNED: bool = False  # log HEAD fallback once per backoff window


def is_fixes_backoff_active() -> bool:
    """Return True if the fixes index is currently in a 429 rate-limit backoff."""
    return time.time() < _FIXES_INDEX_BACKOFF_UNTIL


def _fetch_fixes_index() -> Optional[Dict]:
    """Fetch and cache the fixes index JSON. Returns None on failure."""
    global _fixes_index_cache, _fixes_index_fetched_at, _FIXES_INDEX_BACKOFF_UNTIL, _HEAD_FALLBACK_WARNED
    now = time.time()

    with _fixes_index_lock:
        if _FIXES_INDEX_BACKOFF_UNTIL > now:
            return _fixes_index_cache  # 429 backoff active  --  stale or None
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            with _fixes_index_lock:
                ...
                _FIXES_INDEX_BACKOFF_UNTIL = 0  # clear any leftover backoff
                _HEAD_FALLBACK_WARNED = False    # reset warn flag on successful refresh
            logger.log(f"LuaTools: Fixes index loaded ({len(generic_set)} generic, {len(online_set)} online)")
            return index
        else:
            if resp.status_code == 429:
                _FIXES_INDEX_BACKOFF_UNTIL = now + _FIXES_INDEX_BACKOFF_TTL
                logger.warn(
                    f"LuaTools: Fixes index fetch returned 429 (rate limited)  --  "
                    f"backing off for {_FIXES_INDEX_BACKOFF_TTL}s"
                )
            else:
                logger.warn(f"LuaTools: Fixes index fetch returned {resp.status_code}")
    except Exception as exc:
        exc_str = str(exc)
        if "10061" in exc_str or "connection refused" in exc_str.lower():
            # Network unavailable  --  short backoff (30s) to avoid hammering on every page load
            _FIXES_INDEX_BACKOFF_UNTIL = now + 30
...
def _is_safe_path(base_path: str, target_path: str) -> bool:
    ...
    try:
        abs_base = os.path.normcase(os.path.realpath(base_path))
        abs_target = os.path.normcase(os.path.realpath(os.path.join(base_path, target_path)))
...
def check_for_fixes(appid: int) -> str:
    ...
    if index is not None:
        ...
        # Index unavailable  --  check if we're in a rate-limit backoff window
        global _HEAD_FALLBACK_WARNED
        if is_fixes_backoff_active():
            # Don't hammer R2 with HEAD requests while rate-limited.
            # Return a "rate_limited" sentinel so the frontend can cache the result
            # and suppress repeated backend calls for the backoff duration.
            if not _HEAD_FALLBACK_WARNED:
                remaining = int(_FIXES_INDEX_BACKOFF_UNTIL - time.time())
                logger.log(
                    f"LuaTools: Fixes index rate-limited, skipping HEAD fallback "
                    f"({remaining}s remaining in backoff)"
                )
                _HEAD_FALLBACK_WARNED = True
            result["rateLimited"] = True
            result["backoffRemaining"] = max(0, int(_FIXES_INDEX_BACKOFF_UNTIL - time.time()))
            return json.dumps(result)

        # Genuine miss (index not yet loaded, short outage)  --  fall back to HEAD
    ...
    try:
        ...
        if validate_zip_archive is not None:
            try:
                ...
                is_valid, validation_error = validate_zip_archive(archive_bytes, str(appid))
                if not is_valid:
                    raise RuntimeError(f"Unsafe fix archive: {validation_error}")
            except RuntimeError:
                raise
            except Exception as exc:
                logger.warn(f"LuaTools: Fix ZIP validation failed to run for {appid}: {exc}")
