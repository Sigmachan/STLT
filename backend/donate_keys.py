...
def _load_donated_appids() -> set:
    """Load the set of already-donated app IDs from the local cache file."""
...
def _check_cache_staleness() -> None:
    """Wipe the donated-appids cache if it is older than 7 days."""
    today = date.today()
    ...
    try:
        ...
        if first_line.startswith("DATE:"):
            try:
                cached_date = date.fromisoformat(first_line.split("DATE:")[1])
                if today - cached_date >= timedelta(days=7):
                    logger.log(
                        f"LuaTools: Donation cache is {today - cached_date} old "
                        f"(since {cached_date}), wiping."
                    )
                    with open(DONATED_APPIDS_FILE, "w", encoding="utf-8") as f:
                        f.write(f"DATE:{today.isoformat()}\n")
            except (ValueError, IndexError):
                logger.log("LuaTools: Donation cache date format invalid, wiping.")
                with open(DONATED_APPIDS_FILE, "w", encoding="utf-8") as f:
                    f.write(f"DATE:{today.isoformat()}\n")
        else:
            logger.log("LuaTools: Donation cache missing date header, wiping.")
            with open(DONATED_APPIDS_FILE, "w", encoding="utf-8") as f:
                f.write(f"DATE:{today.isoformat()}\n")
    except Exception as exc:
        logger.warn(f"LuaTools: Failed to check donation cache staleness: {exc}")


def _save_donated_appids(appids: set) -> None:
    """Append newly donated app IDs to the local cache file."""
    try:
...
def send_donation_keys(pairs: List[Tuple[str, str]]) -> bool:
    ...
    try:
        # ── Dedup: skip already-donated pairs (7-day rolling cache) ────
        ...
        logger.log(
            f"LuaTools: Donated AppIDs : {len(new_pairs)} - Resp : {status_code} "
            f"({len(pairs) - len(new_pairs)} already cached, skipped)"
        )
