...
SETTINGS_GROUPS: List[SettingGroup] = [
    ...
    SettingGroup(
        ...
        options=[
            ...
            SettingOption(
                key="sentinelEnabled",
                label="Enable Sentinel (background watch)",
                option_type="toggle",
                description="Run LuaTools Sentinel in the background to detect new installs and manifest staleness.",
                default=False,
                metadata={"yesLabel": "Enabled", "noLabel": "Disabled"},
            ),
            SettingOption(
                key="sentinelIntervalMinutes",
                label="Sentinel Poll Interval (minutes)",
                option_type="number",
                description="How often Sentinel scans for changes. Lower values increase responsiveness but use more resources.",
                default=15,
                metadata={"min": 1, "max": 1440},
            ),
            SettingOption(
                key="sentinelAutoApplyPolicy",
                label="Sentinel Auto-Apply Policy",
                option_type="select",
                description="What Sentinel may do automatically when it finds updates/fixes.",
                default="ask",
                choices=[
                    {"value": "never", "label": "Never auto-apply"},
                    {"value": "ask", "label": "Ask before applying"},
                    {"value": "auto_minor", "label": "Auto-apply minor updates only"},
                ],
            ),
            SettingOption(
                key="githubToken",
